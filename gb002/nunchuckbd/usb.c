        ...
        case GET_DESCRIPTOR:
            ...
            switch (USB_setup.wValue.b[1]) {
                ...
                case HID:
                    switch (USB_setup.wValue.b[0]) {
                        case 0:
                            USB_request.data_ptr = Configuration1 + 0x12;   // add offset of HID1 descriptor from the beginning of Configuration1 descriptor
                            break;
                        case 1:
                            USB_request.data_ptr = Configuration1 + 0x2B;   // add offset of HID2 descriptor from the beginning of Configuration1 descriptor
                            break;
                        default:
                            USB_error_flags |= 0x01;    // set Request Error Flag
                    }
                    ...
                    break;
                case REPORT:
                    switch (USB_setup.wIndex.b[0]) {
                        case 0:
                            USB_request.bytes_left.w = 0x3F;    // total length of Report1 descirptor in HID1 descriptor, rather than Report1
                            USB_request.data_ptr = Report1;
                            break;
                        case 1:
                            USB_request.bytes_left.w = 0x32;    // total length of Report2 descriptor in HID2 descriptor, rather than Report2
                            USB_request.data_ptr = Report2;
                            break;
                        default:
                            USB_error_flags |= 0x01;    // set Request Error Flag
                    }
            ...
            if (USB_setup.wValue.b[0]<=NUM_CONFIGURATIONS) {
                ...
                switch (USB_curr_config = USB_setup.wValue.b[0]) {
                    ...
                    default:
                        ...
                        BD[EP1IN].bytecount = 0x08;         // set EP1 IN byte count to 8 (keyboard report size)
                        BD[EP1IN].address = EP1_IN_buffer;  // set EP1 IN buffer address
                        BD[EP1IN].status = 0x48;            // clear UOWN bit (PIC can write EP1 IN buffer)
                        U1EP1 = ENDPT_IN_ONLY;              // enable EP1 for interrupt in transfers
                        BD[EP2IN].bytecount = 0x03;         // set EP2 IN byte count to 3 (mouse report size)
                        BD[EP2IN].address = EP2_IN_buffer;  // set EP2 IN buffer address
                        BD[EP2IN].status = 0x48;            // clear UOWN bit (PIC can write EP2 IN buffer)
                        U1EP2 = ENDPT_IN_ONLY;              // enable EP2 for interrupt in transfers
#ifdef SHOW_ENUM_STATUS
                        PORTB &= 0xE0;
                        PORTBbits.RB3 = 1;
#endif
